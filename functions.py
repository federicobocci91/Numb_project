...
def parameters(onecell=False, numb=False, mir34=False):
	...
	dic =  {'km' : 5.0e-1
		,'kP' : 1.0e+2
		   ...
		   ,'ldf': 3.0,    'ljf': 0.3,'It' : 0.0}

	if numb:
		if mir34:
			dic.update({ 'U_trans' : 500., 'U_N' : 0.75e+6, 'lU' : 0.5, 'nU' : 2, 'nN' : 2, 'lN' : 0.8, 'gU' : 1.0e-1, 'sU' : 1.0e+2})
		else:
			dic.update({ 'U0' : 1.e+5, 'U_N' : 0.75e+6, 'lU' : 0.5, 'nU' : 2, 'nN' : 2, 'lN' : 0.8, 'gU' : 1.0e-1, 'sU' : 1.0e+2})
...
def equations(onecell=False, numb=False, mir34=False):
	
	if numb:
		if mir34:
			if onecell:
				return {'W' : 'gu200*HS(Z,Z0u200,nZu200,lZu200)*HS(S,S0u200,nSu200,lSu200) - gZ*HS(Z,Z0Z,nZZ,lZZ)*HS(S,S0Z,nSZ,lSZ)*Py(W,km,6) - gJ*HS(I,I0,p,lj)*Py(W,km,5) - ku200*W','Y' : 'gu34*HS(S,S0u34,nSu34,lSu34)*HS(Z,Z0u34,nu34,lZu34) - gS*HS(S,S0S,nSS,lSS)*HS(I,I0S,nI,lIS)*HS(It,I0S,nI,lIS)*Py(Y,km,2) - gN*HS(I,I0,p,ln)*Py(Y,km,2) - U_trans*HS(I,sU,nU,lU)*Py(Y,km,1) - gD*HS(I,I0,p,ld)*Py(Y,km,3) - ku34*Y','Z' : 'kP*gZ*HS(Z,Z0Z,nZZ,lZZ)*HS(S,S0Z,nSZ,lSZ)*Pl(W,km,6)                    - kZ*Z','S' : 'kP*gS*HS(S,S0S,nSS,lSS)*HS(I,I0S,nI ,lIS)*HS(It,I0S,nI,lIS)*Pl(Y,km,2)  - kS*S','N' : 'kP*gN*HS(I,I0,p,ln)*Pl(Y,km,2)*HS(U,U_N,nN,lN) - N*( (kc*D + kt*Dt)*HS(I,I0,pf,ldf) + (kc*J + kt*Jt)*HS(I,I0,pf,ljf) ) - k*N','D' : 'kP*gD*HS(I,I0,p,ld)*Pl(Y,km,3) - D*(  kc*N*HS(I,I0,pf,ldf) + kt*Nt ) - k*D','J' : 'kP*gJ*HS(I,I0,p,lj)*Pl(W,km,5) - J*(  kc*N*HS(I,I0,pf,ljf) + kt*Nt ) - k*J','I' : 'kt*N*( Dt*HS(I,I0,pf,ldf) + Jt*HS(I,I0,pf,ljf) ) - kI*I','U' : 'kP*U_trans*HS(I,sU,nU,lU)*Pl(Y,km,1) - gU*U'}
			else:
				return {'W' : 'gu200*HS(Z,Z0u200,nZu200,lZu200)*HS(S,S0u200,nSu200,lSu200) - gZ*HS(Z,Z0Z,nZZ,lZZ)*HS(S,S0Z,nSZ,lSZ)*Py(W,6,km,u0200) - gJ*HS(I,I0,p,lj)*Py(W,5,km,u0200) - ku200*W','Y' : 'gu34*HS(S,S0u34,nSu34,lSu34)*HS(Z,Z0u34,nu34,lZu34) - gS*HS(S,S0S,nSS,lSS)*HS(I,I0S,nI,lIS)*HS(It,I0S,nI,lIS)*Py(Y,2,km,u034) - gN*HS(I,I0,p,ln)*Py(Y,2,km,u034) - U_trans*HS(I,sU,nU,lU)*Py(Y,1,km,u034) - gD*HS(I,I0,p,ld)*Py(Y,3,km,u034) - ku34*Y','Z' : 'kP*gZ*HS(Z,Z0Z,nZZ,lZZ)*HS(S,S0Z,nSZ,lSZ)*Pl(W,6,km,u0200) - kZ*Z','S' : 'kP*gS*HS(S,S0S,nSS,lSS)*HS(I,I0S,nI ,lIS)*HS(It,I0S,nI,lIS)*Pl(Y,2,km,u034)  - kS*S','N' : 'kP*gN*HS(I,I0,p,ln)*Pl(Y,2,km,u034)*HS(U,U_N,nN,lN)  - N*( (kc*D + kt*(Dn + Dt))*HS(I,I0,pf,ldf) + (kc*J + kt*(Jn + Jt))*HS(I,I0,pf,ljf) ) - k*N','D' : 'kP*gD*HS(I,I0,p,ld)*Pl(Y,3,km,u034)  - D*(  kc*N*HS(I,I0,pf,ldf) + kt*(Nn + Nt) )- k*D','J' : 'kP*gJ*HS(I,I0,p,lj)*Pl(W,5,km,u0200) - J*(  kc*N*HS(I,I0,pf,ljf) + kt*(Nn + Nt) ) - k*J','I' : 'kt*N*( (Dn + Dt)*HS(I,I0,pf,ldf) + (Jn + Jt)*HS(I,I0,pf,ljf) ) - kI*I','U' : 'kP*U_trans*HS(I,sU,nU,lU)*Pl(Y,1,km,u034) - gU*U'}
		else:
			if onecell:
				return {'W' : 'gu200*HS(Z,Z0u200,nZu200,lZu200)*HS(S,S0u200,nSu200,lSu200) - gZ*HS(Z,Z0Z,nZZ,lZZ)*HS(S,S0Z,nSZ,lSZ)*Py(W,km,6) - gJ*HS(I,I0,p,lj)*Py(W,km,5) - ku200*W','Y' : 'gu34*HS(S,S0u34,nSu34,lSu34)*HS(Z,Z0u34,nu34,lZu34) - gS*HS(S,S0S,nSS,lSS)*HS(I,I0S,nI,lIS)*HS(It,I0S,nI,lIS)*Py(Y,km,2) - gN*HS(I,I0,p,ln)*Py(Y,km,2) - gD*HS(I,I0,p,ld)*Py(Y,km,3) - ku34*Y','Z' : 'kP*gZ*HS(Z,Z0Z,nZZ,lZZ)*HS(S,S0Z,nSZ,lSZ)*Pl(W,km,6)                    - kZ*Z','S' : 'kP*gS*HS(S,S0S,nSS,lSS)*HS(I,I0S,nI ,lIS)*HS(It,I0S,nI,lIS)*Pl(Y,km,2)  - kS*S','N' : 'kP*gN*HS(I,I0,p,ln)*Pl(Y,km,2)*HS(U,U_N,nN,lN) - N*( (kc*D + kt*Dt)*HS(I,I0,pf,ldf) + (kc*J + kt*Jt)*HS(I,I0,pf,ljf) ) - k*N','D' : 'kP*gD*HS(I,I0,p,ld)*Pl(Y,km,3) - D*(  kc*N*HS(I,I0,pf,ldf) + kt*Nt ) - k*D','J' : 'kP*gJ*HS(I,I0,p,lj)*Pl(W,km,5) - J*(  kc*N*HS(I,I0,pf,ljf) + kt*Nt ) - k*J','I' : 'kt*N*( Dt*HS(I,I0,pf,ldf) + Jt*HS(I,I0,pf,ljf) ) - kI*I','U' : 'U0*HS(I,sU,nU,lU) - gU*U'}
			else:
				return {'W' : 'gu200*HS(Z,Z0u200,nZu200,lZu200)*HS(S,S0u200,nSu200,lSu200) - gZ*HS(Z,Z0Z,nZZ,lZZ)*HS(S,S0Z,nSZ,lSZ)*Py(W,6,km,u0200) - gJ*HS(I,I0,p,lj)*Py(W,5,km,u0200) - ku200*W','Y' : 'gu34*HS(S,S0u34,nSu34,lSu34)*HS(Z,Z0u34,nu34,lZu34) - gS*HS(S,S0S,nSS,lSS)*HS(I,I0S,nI,lIS)*HS(It,I0S,nI,lIS)*Py(Y,2,km,u034) - gN*HS(I,I0,p,ln)*Py(Y,2,km,u034) - gD*HS(I,I0,p,ld)*Py(Y,3,km,u034) - ku34*Y','Z' : 'kP*gZ*HS(Z,Z0Z,nZZ,lZZ)*HS(S,S0Z,nSZ,lSZ)*Pl(W,6,km,u0200) - kZ*Z','S' : 'kP*gS*HS(S,S0S,nSS,lSS)*HS(I,I0S,nI ,lIS)*HS(It,I0S,nI,lIS)*Pl(Y,2,km,u034)  - kS*S','N' : 'kP*gN*HS(I,I0,p,ln)*Pl(Y,2,km,u034)*HS(U,U_N,nN,lN)  - N*( (kc*D + kt*(Dn + Dt))*HS(I,I0,pf,ldf) + (kc*J + kt*(Jn + Jt))*HS(I,I0,pf,ljf) ) - k*N','D' : 'kP*gD*HS(I,I0,p,ld)*Pl(Y,3,km,u034)  - D*(  kc*N*HS(I,I0,pf,ldf) + kt*(Nn + Nt) )- k*D','J' : 'kP*gJ*HS(I,I0,p,lj)*Pl(W,5,km,u0200) - J*(  kc*N*HS(I,I0,pf,ljf) + kt*(Nn + Nt) ) - k*J','I' : 'kt*N*( (Dn + Dt)*HS(I,I0,pf,ldf) + (Jn + Jt)*HS(I,I0,pf,ljf) ) - kI*I','U' : 'U0*HS(I,sU,nU,lU) - gU*U'}

	else:
		if onecell:
			return {'W' : 'gu200*HS(Z,Z0u200,nZu200,lZu200)*HS(S,S0u200,nSu200,lSu200) - gZ*HS(Z,Z0Z,nZZ,lZZ)*HS(S,S0Z,nSZ,lSZ)*Py(W,km,6) - gJ*HS(I,I0,p,lj)*Py(W,km,5) - ku200*W','Y' : 'gu34*HS(S,S0u34,nSu34,lSu34)*HS(Z,Z0u34,nu34,lZu34) - gS*HS(S,S0S,nSS,lSS)*HS(I,I0S,nI,lIS)*HS(It,I0S,nI,lIS)*Py(Y,km,2) - gN*HS(I,I0,p,ln)*Py(Y,km,2) - gD*HS(I,I0,p,ld)*Py(Y,km,3) - ku34*Y','Z' : 'kP*gZ*HS(Z,Z0Z,nZZ,lZZ)*HS(S,S0Z,nSZ,lSZ)*Pl(W,km,6)                    - kZ*Z','S' : 'kP*gS*HS(S,S0S,nSS,lSS)*HS(I,I0S,nI ,lIS)*HS(It,I0S,nI,lIS)*Pl(Y,km,2)  - kS*S','N' : 'kP*gN*HS(I,I0,p,ln)*Pl(Y,km,2) - N*( (kc*D + kt*Dt)*HS(I,I0,pf,ldf) + (kc*J + kt*Jt)*HS(I,I0,pf,ljf) ) - k*N','D' : 'kP*gD*HS(I,I0,p,ld)*Pl(Y,km,3) - D*(  kc*N*HS(I,I0,pf,ldf) + kt*Nt ) - k*D','J' : 'kP*gJ*HS(I,I0,p,lj)*Pl(W,km,5) - J*(  kc*N*HS(I,I0,pf,ljf) + kt*Nt ) - k*J','I' : 'kt*N*( Dt*HS(I,I0,pf,ldf) + Jt*HS(I,I0,pf,ljf) ) - kI*I'}
		else:
			return {'W' : 'gu200*HS(Z,Z0u200,nZu200,lZu200)*HS(S,S0u200,nSu200,lSu200) - gZ*HS(Z,Z0Z,nZZ,lZZ)*HS(S,S0Z,nSZ,lSZ)*Py(W,6,km,u0200) - gJ*HS(I,I0,p,lj)*Py(W,5,km,u0200) - ku200*W','Y' : 'gu34*HS(S,S0u34,nSu34,lSu34)*HS(Z,Z0u34,nu34,lZu34) - gS*HS(S,S0S,nSS,lSS)*HS(I,I0S,nI,lIS)*HS(It,I0S,nI,lIS)*Py(Y,2,km,u034) - gN*HS(I,I0,p,ln)*Py(Y,2,km,u034) - gD*HS(I,I0,p,ld)*Py(Y,3,km,u034) - ku34*Y','Z' : 'kP*gZ*HS(Z,Z0Z,nZZ,lZZ)*HS(S,S0Z,nSZ,lSZ)*Pl(W,6,km,u0200) - kZ*Z','S' : 'kP*gS*HS(S,S0S,nSS,lSS)*HS(I,I0S,nI ,lIS)*HS(It,I0S,nI,lIS)*Pl(Y,2,km,u034)  - kS*S','N' : 'kP*gN*HS(I,I0,p,ln)*Pl(Y,2,km,u034)  - N*( (kc*D + kt*(Dn + Dt))*HS(I,I0,pf,ldf) + (kc*J + kt*(Jn + Jt))*HS(I,I0,pf,ljf) ) - k*N','D' : 'kP*gD*HS(I,I0,p,ld)*Pl(Y,3,km,u034)  - D*(  kc*N*HS(I,I0,pf,ldf) + kt*(Nn + Nt) )- k*D','J' : 'kP*gJ*HS(I,I0,p,lj)*Pl(W,5,km,u0200) - J*(  kc*N*HS(I,I0,pf,ljf) + kt*(Nn + Nt) ) - k*J','I' : 'kt*N*( (Dn + Dt)*HS(I,I0,pf,ldf) + (Jn + Jt)*HS(I,I0,pf,ljf) ) - kI*I'}
...
def vlim(c=None, numb=False):
	
	if numb:
	
		if c=='E':
			return {'N': [0.0e+4, 3.0e+4], 'D': [0.0e+4, 0.2e+4], 'J': [0.0e+4, 1.0e+4], 'I': [0.0e+4, 0.002e+4]
			,'W': [1.5e+4, 3.5e+4], 'Z': [0.0e+5, 0.1e+4], 'Y': [1.6e+4, 2.2e+4], 'S': [0.5e+5, 2.0e+5],'U': [0.5e+6, 1.0e+6]
			}
		elif c=='E/M':
			return {'N': [3.0e+4, 4.0e+4], 'D': [0.0e+4, 0.1e+4], 'J': [0.1e+3, 1.0e+3], 'I': [0.5e+3, 1.5e+3]
			,'W': [0.5e+4, 1.5e+4], 'Z': [1.0e+4, 1.5e+5], 'Y': [1.5e+4, 1.7e+4], 'S': [1.7e+5, 2.1e+5],'U': [0.5e+6, 1.0e+6]
			}
		elif c=='M':
			return {'N': [2.0e+4, 3.0e+4], 'D': [0.0e+4, 0.1e+4], 'J': [2.0e+3, 5.0e+3], 'I': [0.4e+3, 1.6e+3]
			,'W': [0.0e+4, 0.5e+4], 'Z': [4.0e+5, 8.5e+5], 'Y': [0.6e+4, 1.5e+4], 'S': [2.0e+5, 3.0e+5],'U': [0.5e+6, 1.0e+6]
			}
		else:
			return {'N': [0.0e+4, 3.0e+4], 'D': [0.0e+4, 0.2e+4], 'J': [0.0e+4, 1.0e+4], 'I': [0.0e+4, 0.2e+4]
			,'W': [0.0e+4, 4.0e+4], 'Z': [0.0e+5, 9.0e+5], 'Y': [0.6e+4, 2.4e+4], 'S': [5.0e+4, 3.0e+5],'U': [0.5e+6, 1.0e+6]
			}
...
def nsignal_sum(p, pts, k, key, hexagonal=True):
	n = p['n']
	fng_dic={'D': 'HS(I,I0,pf,ldf)','J': 'HS(I,I0,pf,ljf)'}
	fng = 1.0
	if k=='N':
		fng = eval(fng_dic[key],p,pts)
	X = periodic_bcondition(pts[k]*fng, n)
	if hexagonal:
		return (1.0/6.0)*( X[0:n,1:(n+1)] + X[1:(n+1),0:n] + X[2:(n+2),1:(n+1)] + X[1:(n+1),2:(n+2)] + X[0:n,2:(n+2)] + X[2:(n+2),2:(n+2)] )
...
def plot_fractionStates(eqs, p, v, r_v, key, tr, pts_i=None, vlim=None, l=['M','E/M','E'], c=['#e8656c','#e4fc36','#00ff9c'],
						show_snapshot=False, nsignal_dict={'N': ['D', 'J'], 'I': ['D', 'J'], 'D': ['N'], 'J': ['N']}, fig_name=None, title=None):
	fs = []
	for i in range(len(r_v)):
		if v=='fng':
			p['ldf'] = 1.0 + 4.0*r_v[i]
...
def plot_continuation(ODE, freepar, keys, bif_startpoint, LocBifPoints=['LP','B'], PCargs=None, returnLP=None, ics=None,
					  ...
					  fs=[6,5], fontsize=18, fig_name=False, title=None):
	...
	if PCargs==None:
		PCargs = PyCont_args(ODE.name, freepar, maxpoints, saveeigen=True, LocBifPoints=LocBifPoints,maxstep=maxstep, minstep=minstep, step=step)
...
def plot_phasediagram(ODE, freepar, v, r_v, bif_startpoint, keys=False, xlim=False, ylim=False, xticks=False, yticks=False,
	...
	for i in r_v:
		...
		lp = plot_continuation(ODE, freepar, keys, bif_startpoint, PCargs=PCargs, returnLP=BifPoints, showcurve=show_continuation, ics=[fp],fs=[4,3], fontsize=12,nrow=nrow, ncol=ncol)
		x += [lp]
		plt.show()
	if silence:
		sys.stdout = original_stdout
...
def nullclines(axis, DSargs, stab, fp, nfp=0, vlim=None, c = ['b','g'], maxpoints=[1000,1000], step=5e+1, minstep=1e-1,maxstep=1e+3, fs=[6,5], fig_name=False, plotaxis=[0,1], loc=0, fontsize=18, pcontour=None, silence=False,xticks=False, yticks=False):
...
def param_sensitivity_bifurcations(DSargs, freepar, key, list_pars, bif_startpoint, d=[0.0, 0.1, -0.1], c=['k', 'b', 'r'],
	...
	for i in range(len(list_pars)):
		...
		for j in range(len(d)):
			...
			PCargs = PyCont_args('psensit', freepar, 200, saveeigen=True, maxstep=maxstep, minstep=minstep, step=step,LocBifPoints=['B','LP'])
			PyCont = PyDSTool.ContClass(ODE)
			PyCont.newCurve(PCargs)
...
def plot_3Dpotential(x1, x2, npoints, xlim=False, ylim=False, zlim=False, offset=5, cut=9.5, fig_name=False,nbins=100, scale=1000.0):
